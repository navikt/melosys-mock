...
fun Route.euxApi() {
    ...
    get { req: Buc ->
        call.respond(status = HttpStatusCode.OK, message = ObjectMapper().readTree(FileUtils.hentBuc(req.bucID)))
    }
    delete<Buc> {
    ...
    get { req: Sed ->
        call.respond(status = HttpStatusCode.OK, message = ObjectMapper().readTree(FileUtils.hentSed(req.bucID, req.sedID)))
    }
    ...
    class Institusjoner
    get { _: Institusjoner ->
        call.respond(HttpStatusCode.OK, ObjectMapper().readTree(FileUtils.hentInstitusjoner()))
    }
}
